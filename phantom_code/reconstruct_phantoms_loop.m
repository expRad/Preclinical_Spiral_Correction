...
for loop = 1:length(measurements)
    ...
    gstf_y_del = exp(1i*2*pi*linspace(H_1.f_axis(1),H_1.f_axis(end),size(grad_x_ZF,1)).' *delay);
    gstf_z_del = exp(1i*2*pi*linspace(H_1.f_axis(1),H_1.f_axis(end),size(grad_x_ZF,1)).' *delay);
...
%% Plot reconstructed images (Figure 3)
...
annotation('arrow',0.3+[0 -0.02],0.195+[0 -0.02],'Color','w','LineWidth',1.5);
axis image; axis off;
xticklabels([]); yticklabels([]);
...
annotation('arrow',0.61+[0 -0.02],0.195+[0 -0.02],'Color','w','LineWidth',1.5);
axis image; axis off;
...
annotation('arrow',0.70+[0 0.03],0.15+[0 -0.01],'Color','w','LineWidth',1.5);
annotation('arrow',0.778+[0 0.0],0.163+[0 -0.023],'Color','k','LineWidth',1.5);
axis image; axis off;
...
leg = legend('numColumns',4,'Position',[0.3 0.92 0.68 0.04]);
ylabel('image intensity (a.u.)');
set(gca,'FontName','Times','Fontsize',9);
...
hold on;
plot(meas.traj_nom(1:end-meas.nPost,1), 'LineWidth',1.8,'Color',yellow);
xlim([-1 1]*3.5);
ylim([-1 1]*3.5);
axis off;
title('spiral trajectory','FontSize',7.8,'FontWeight','normal');
...
ax2 = axes('Position',[0.34 0.45 0.33 0.5]);
imagesc(abs(recos_nom(x1:x2,y1:y2,2)), c_lims2a);
axis image; axis off;
title('uncorrected','FontSize',7.8,'FontWeight','normal');

ax3 = axes('Position',[0.67 0.45 0.33 0.5]);
imagesc(abs(recos_del(x1:x2,y1:y2,2)), c_lims2a);
axis image; axis off;
title('delay correction','FontSize',7.8,'FontWeight','normal');

ax4 = axes('Position',[0.01 -0.05 0.33 0.5]);
imagesc(abs(recos_girf(x1:x2,y1:y2,2)), c_lims2a);
axis image; axis off;
title('GSTF correction','FontSize',7.8,'FontWeight','normal');

ax5 = axes('Position',[0.34 -0.05 0.33 0.5]);
imagesc(abs(recos_girfdel(x1:x2,y1:y2,2)), c_lims2a);
axis image; axis off;
title('GSTF+delay','FontSize',7.8,'FontWeight','normal');

ax6 = axes('Position',[0.67 -0.05 0.33 0.5]);
imagesc(abs(recos_meas(x1:x2,y1:y2,2)), c_lims2a);
axis image; axis off;
title('ground truth','FontSize',7.8,'FontWeight','normal');
