...
% comment in to reconstruct phantom data acquired with 96 interleaves:
filepath = '..\phantom_data\spiral_data_p96.mat';
path2save = '..\phantom_data\RMSEs_and_delays_p96.mat';

% comment in to reconstruct phantom data acquired with 16 interleaves:
% filepath = '..\phantom_data\spiral_data_p16.mat';
% path2save = '..\phantom_data\RMSEs_and_delays_p16.mat';

% comment in to reconstruct phantom data acquired with 3 interleaves:
% filepath = '..\phantom_data\spiral_data_p03.mat';
% path2save = '..\phantom_data\RMSEs_and_delays_p03.mat';
...
for loop = 1:length(delays_girf)
    ...
    delay_x = delays_girf(loop);
    delay_x2 = delays_del(loop);
    offset_x = 0;
    offset_y = 0;
    offset_del_x = 0;
    offset_del_y = 0;
    ...
    % calculate in-plane gradient waveforms for each spiral interleaf
    ...
    t_GIRF = H_1.t_axis;
    if t_GIRF(end) < t_ADC(end)
        N = ceil(t_ADC(end)/H_1.dt);
        t_GIRF = ((1:N)-0.5)*H_1.dt;
    end
    ...
    grad_px_del = real(ifft1d( tmp1.*tmp2, 1)) + offset_del_x;
    grad_px_del = grad_px_del(nExtra+1:end-nExtra,:);

    tmp1 = fft1d(grad_y_ZF,1);
    tmp2 = repmat(gstf_y_del, [1,spiral_data.numInterleaves]);
    grad_py_del = real(ifft1d( tmp1.*tmp2, 1)) + offset_del_y;
...
%% Fit polynomial to nRMSE-vs-delay
...
params_del = lsqcurvefit(quad_function, initial_guess, delays_del(1:11)*1e5, sum_abs_err_del(1:11)*100); % Fit works better when we multiply argument by 100
...
fit_del = fit_del/100; % Reverse multiplication by 100
